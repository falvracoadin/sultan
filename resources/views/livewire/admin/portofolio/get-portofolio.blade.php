<div>
    ...
    <div class="container-fluid">
        <main class="tm-main">
            ...
            <div wire:ignore class="row tm-row">
                <div class="col-md-6">
                    <a wire:click="addPortofolio"><button type="button" class="btn green">Tambah Portofolio</button></a>
                </div>    
                <div class="col-md-6">
                    <a wire:click='addKategori'><button type="button" class="btn green">Tambah Kategori Portofolio</button></a>
                </div>
            </div>         
            <div class="row tm-row">
                @foreach($portofolios as $ind => $art)
                <article class="col-12 col-md-6 tm-post">
                    <hr class="tm-hr-primary">
                    <a class="effect-lily tm-post-link tm-pt-60">
                        <div class="tm-post-link-inner">
                            <img src="{{asset($art['file'] == null ? 'admin/img/img-01.jpg' : 'storage/'.$art['file'])}}" alt="Image" class="img-fluid">                            
                        </div>
                        <span wire:click='delete({{$art['id']}})' class="position-absolute tm-new-badge">delete</span>
                        <h2 wire:click="showUpdatePortofolio({{$art['id']}})"  class="tm-pt-30 tm-color-primary tm-post-title">{{$art['nama_portofolio']}}</h2>
                    </a>
                    <div wire:click="showUpdatePortofolio({{$art['id']}})" style="cursor: pointer;">             
                        <p class="tm-pt-30">
                            {{substr($art['deskripsi'], 0, 200)}}
                        </p>
                        <div class="d-flex justify-content-between tm-pt-45">
                            <span class="tm-color-primary">{{$art['kategori']}}</span>
                            <span class="tm-color-primary">{{date('d M Y', strtotime($art['date']))}}</span>
                        </div>
                        <hr>
    ...
    @livewire('admin.portofolio.update-portofolio', ['id' => $updateId])
    @livewire('admin.portofolio.create-portofolio')
    @livewire('admin.portofolio.create-kategori-portofolio')
    @if($delete !== null)
        ...
        <div class="modal-dialog modal-dialog-centered">
          <div class="modal-content">
            ...
            <div class="modal-body">
              <p>Anda yakin ingin menghapus portofolio?</p>
            </div>
